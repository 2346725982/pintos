...
void process_exit(void) {
	...
	if (cur->ELF_file != NULL) {
		file_close(cur->ELF_file);
	}
...
static bool setup_stack(void **esp, const char* file_name, char** save_ptr) {
	...
	char **tmp_argv = malloc(DEFAULT_ARGV * sizeof(char *));
	int i, argc = 0, tmp_argc = 0;
	int tmp_argv_size = DEFAULT_ARGV, argv_size = DEFAULT_ARGV;

	for (token = (char *) file_name; token != NULL;
			token = strtok_r(NULL, " ", save_ptr)) {
		tmp_argv[tmp_argc] = token;
		tmp_argc++;
		// Resize argv
		if (tmp_argc >= tmp_argv_size) {
			tmp_argv_size *= 2;
			tmp_argv = realloc(tmp_argv, tmp_argv_size * sizeof(char *));
			argv = realloc(argv, tmp_argv_size * sizeof(char *));
		}
	}
	...
	for (; tmp_argc >= 0; tmp_argc--) {
		token = tmp_argv[tmp_argc];
		*esp -= strlen(token) + 1;
		argv[tmp_argc] = *esp;
		argc++;
